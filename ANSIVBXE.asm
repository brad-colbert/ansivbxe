...
; VBXE equates
vbxe_mem_base	equ	$A000			; If I put it here, it should be OK and it won't conflict with the extended RAM.

vbxe_screen_top	equ	vbxe_mem_base + 256	; points to where the first character of the screen would be

recv_buffer	equ	end			; first free space after the program.

send_buffer	equ	recv_buffer + 256	; first free space after the receive buffer. This will be 256 characters for the send
...
ctrl_seq_buf	equ	send_buffer + 256	; a buffer for the control sequence. I'll have to mess around with various sizes for
						; this. 256 bytes seems way overkill, so I probably only need 16 or so. For now, it's
						; 256 though.

; Display Handler Equates (among others)

cursor_address	equ	$80			; points to the address where the text cursor points
text_color	equ	$82			; current text color to be put on the screen.
row		equ	$83			; cursor row
column		equ	$84			; cursor column

src_ptr		equ	$85			; temporary source pointer on the zero page for memory moves
dst_ptr		equ	$87			; same as above, but for destination
counter		equ	$89			; byte counter for above

ctrl_seq_flg	equ	$8B			; bit 7 indicates escape received, bit 6 indicates CSI received.

cursor_flg	equ	$8C			; bit 7 indicates whether or not the cursor is currently visible
						; that is, if it's a 1, then the color of the current character
						; has been inverted to show the cursor.
temp_char	equ	$8D			; holds a character temporarily for the recieve processing routines

temp_key_char	equ	$8E

recvbuflen	equ	$8F			; holds the number of bytes in the receive buffer that are waiting to be processed.

sendbufstart	equ	$90			; holds the position of the last character taken from the send FIFO
sendbufend	equ	$91			; holds the position of the last character added to the send FIFO
...
ctrl_seq_index	equ	$92			; points to the current position in the control sequence buffer.

final_byte	equ	$93			; holds the control sequence final byte
inter_byte	equ	$94			; holds the control sequence intermediate byte
parameter_val	equ	$95			; holds a single parameter value

		ORG	$2800			; start of init routine
...
clear_ram_bcb					; blitter routine to clear the whole VBXE RAM. 
		...
		.long	clear_ram_end-clear_ram_bcb	; don't overwrite the blitter list
...
clear_ram_end
		...
		org	$02E2			; init address

		.word	$2800

		org	$2800			; next segment also begins at $2000
...
wait_for_byte	jsr	check_sendbuf		; check to see if the send buffer has characters to be sent
	...
	.local				; read bytes - right now this will get a max of 255 bytes, even though the buffer is
...
check_sendbuf	.local				; checks to see if the send buffer is empty, and sends it if it's not.
...
; overlay address starts at top of memory - 3840. this means the last line is at the top of memory
		...
		.long	$080000 - [80*24*2]	; overlay address starts at the top of VBXE memory - bytes per screen
